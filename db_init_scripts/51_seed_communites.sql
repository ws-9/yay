-- Seed fixed community roles
INSERT INTO community_roles (name, hierarchy_level, can_manage_channels, can_ban_users, can_manage_roles, can_delete_messages, can_manage_community_settings) VALUES
('Admin', 1, true, true, true, true, true),
('Moderator', 2, true, true, false, true, false),
('Member', 3, false, false, false, false, false);
...
-- Add community members (besides owners) with Member role by default
INSERT INTO community_members (community_id, user_id, role_id)
-- user's community: alice, bob, charlie
SELECT c.id, u.id, (SELECT id FROM community_roles WHERE name = 'Member') FROM communities c, users u 
...
-- Add community owners to their own communities with Admin role
INSERT INTO community_members (community_id, user_id, role_id)
SELECT c.id, c.owner_id, (SELECT id FROM community_roles WHERE name = 'Admin') FROM communities c;

-- Promote first member of each community to Moderator
UPDATE community_members cm
SET role_id = (SELECT id FROM community_roles WHERE name = 'Moderator')
WHERE (cm.community_id, cm.user_id) IN (
    SELECT DISTINCT ON (community_id) community_id, user_id
    FROM community_members
    WHERE user_id NOT IN (SELECT owner_id FROM communities WHERE id = community_id)
