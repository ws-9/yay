...
public class CommunityServiceImpl implements CommunityService {
  ...
  public GetCommunityResponse getCommunity(long id) {
    ...
    CommunityRoleResponse role =
        communityMemberRepository
            .findWithRoleByKey(new CommunityMemberKey(id, userId))
  ...
  public List<GetMemberResponse> getAllMembers(Long id) {
    ...
    if (!isMember && !isAdmin) {
      throw new ResponseStatusException(
          HttpStatus.FORBIDDEN, "You must be a member to view members");
    }

    List<CommunityMember> members =
        communityMemberRepository.findAllWithUserAndRoleByKey_CommunityId(id);
  ...
  public List<GetCommunityResponse> getUserOwnCommunities() {
    ...
    List<Community> communities =
        communityRepository.findAllWithOwnerByMembers_User_id(user.getId());
    ...
    Map<Long, List<GetChannelResponse>> communityIdToChannels =
        channels.stream()
            .map(
                c ->
            ...
            .collect(Collectors.groupingBy(GetChannelResponse::communityId));

    Map<Long, CommunityRoleResponse> communityIdToRole =
        communityMemberRepository
            .findAllWithRoleByKey_UserIdAndKey_CommunityIdIn(user.getId(), communityIds)
            .stream()
            .collect(
                Collectors.toMap(
  ...
  public List<GetChannelResponse> getCommunityChannels(Long communityId) {
    ...
    if (!isMember && !isAdmin) {
      throw new ResponseStatusException(
          HttpStatus.FORBIDDEN, "You must be a member to view channels");
    }
