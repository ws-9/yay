...
export default function ChannelSettingDialog() {
  const channelSettingDialog = useChannelSettingsDialog();
  const { closeChannelSettingsDialog } = useUiStoreActions();

  return (
    <Dialog.Root
      open={
        channelSettingDialog.isOpen && channelSettingDialog.channelId !== null
      }
      ...
      <Dialog.Portal>
        ...
        <Dialog.Popup className="fixed top-1/2 left-1/2 -mt-8 w-96 max-w-[calc(100vw-3rem)] -translate-x-1/2 -translate-y-1/2 rounded-lg bg-gray-50 p-6 text-gray-900 outline outline-1 outline-gray-200 transition-all duration-150 data-[ending-style]:scale-90 data-[ending-style]:opacity-0 data-[starting-style]:scale-90 data-[starting-style]:opacity-0 dark:outline-gray-300">
          ...
          <Dialog.Description className="mb-6 text-base text-gray-600">
            Choose a name for your existing channel
          </Dialog.Description>
          <ChannelNameForm
            channelId={channelSettingDialog.channelId!}
            onSuccess={closeChannelSettingsDialog}
          />
        </Dialog.Popup>
      </Dialog.Portal>
    </Dialog.Root>
